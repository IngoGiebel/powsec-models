...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, frame hidden, borderline west={3pt}{0pt}{shadecolor}, interior hidden, breakable, boxrule=0pt, enhanced]}{\end{tcolorbox}}\fi

\textbf{Authors:} Ingo Giebel, with AI-assisted analysis by Dione 🌙 \&
Inanna ⚔️\\
\textbf{Affiliation:} Independent Researcher\\
\textbf{ORCID:}
\href{https://orcid.org/0009-0005-4238-000X}{0009-0005-4238-000X}\\
...
\textbf{Keywords:} Bitcoin, Proof-of-Work, mining economics, hashrate
...
\subsection{1.1 Bitcoin in a Nutshell}\label{bitcoin-in-a-nutshell}}
...
novel compliance concerns for institutional holders (Wendl et al.,
2025).
...
assessing censorship resistance (Cong, He \& Li, 2021).
...
space usage (Easley, O'Hara \& Basu, 2019; Carter \& Jeng, 2023).
...
that no single prior study has modeled jointly.
...
{\centering \includegraphics[width=1\textwidth,height=\textheight]{../output/bitcoin_mining_flow.png}

}

\caption{Figure 5: Bitcoin Proof-of-Work mining process flow. The
diagram illustrates the core operational cycle: unconfirmed transactions
are selected from the mempool during block assembly, miners compete to
solve a cryptographic puzzle (PoW), and the first valid solution extends
the blockchain. The successful miner receives the block reward
(currently 3.125 BTC after the April 2024 halving) plus accumulated
transaction fees. The lower-left panel shows the halving schedule ---
the block reward is cut in half approximately every 210,000 blocks
(\textasciitilde4 years), from the initial 50 BTC in 2009 to the current
3.125 BTC. The lower-right panel depicts the Difficulty Adjustment
Algorithm (DAA), which recalibrates the puzzle difficulty every 2,016
blocks to maintain the target 10-minute block interval, forming a
negative feedback loop that stabilizes the network regardless of
...
\caption{Figure 6: Interconnected risk vector map showing the four
systemic risks analyzed in this paper and their feedback relationships.
The four primary risk vectors --- Miner Capitulation, Content Pollution,
Hashrate Concentration, and Institutional Exit --- are arranged in a
diamond topology around a central systemic risk node, emphasizing their
interdependence. Red arrows indicate destabilizing feedback loops: price
declines trigger miner exits that increase hashrate concentration, while
content pollution triggers compliance concerns that drive institutional
exits, further depressing prices. Green arrows indicate stabilizing
feedback loops: the Difficulty Adjustment Algorithm lowers the breakeven
cost as miners exit, and rising transaction fees from block congestion
push back against low-value inscriptions. The coexistence of both
destabilizing and stabilizing mechanisms is a central theme of this
paper's analysis.}
...
\caption{Figure 7: Fee-market thermostat mechanism illustrating
Bitcoin's endogenous self-regulation of block space pollution. The
upper-left panel shows the cyclic feedback process: as Ordinals
inscriptions fill blocks, transaction fees rise, pricing out low-value
inscriptions and restoring equilibrium at approximately 40\% saturation.
The upper-right inset plots the fee-pushback function
...
\subsection{2.1 Mining Economics \& Difficulty
...
block rewards and transaction fees. Prat \& Walter (2021) developed a
...
we incorporate in our miner stress simulation. Easley, O'Hara \& Basu
...
\subsection{2.2 Network Concentration \&
...
Coefficient methodology we employ. Cong, He \& Li (2021) analyzed the
...
\subsection{2.3 Content Pollution \& Block Space
...
Ordinals and their impact on block space economics. Carter \& Jeng
...
\subsection{2.4 Market Microstructure \& Institutional
Adoption}\label{market-microstructure-institutional-adoption}}

Almgren \& Chriss (2001) developed the foundational model for optimal
...
framework for Bitcoin's thinner markets. Makarov \& Schoar (2020)
...
\subsection{\texorpdfstring{3.1 Miner Capitulation Stress Test
(\texttt{miner\_stress.py})}{3.1 Miner Capitulation Stress Test (miner\_stress.py)}}\label{miner-capitulation-stress-test-miner_stress.py}}

\hypertarget{model-description}{%
\subsubsection{3.1.1 Model Description}\label{model-description}}
...
\subsubsection{3.1.2 Difficulty Adjustment Algorithm (DAA)
...
(Prat \& Walter, 2021). Our simulation models this as a smoothed partial
adjustment:
...
\subsubsection{3.1.3 Data Sources}\label{data-sources}}
...
\subsubsection{3.1.4 Key Assumptions}\label{key-assumptions}}

\textbf{Table 1: Key Model Assumptions and Ratings.} Summary of core
assumptions underlying the miner stress model, with justifications and
confidence ratings. The weakest assumption---linear extrapolation from
tracked to untracked miners---is flagged as questionable and subjected
to sensitivity analysis in Section 5.
...
\subsection{\texorpdfstring{3.2 Blockchain Content Pollution
(\texttt{content\_pollution.py})}{3.2 Blockchain Content Pollution (content\_pollution.py)}}\label{blockchain-content-pollution-content_pollution.py}}

\hypertarget{model-description-1}{%
\subsubsection{3.2.1 Model Description}\label{model-description-1}}
...
\subsubsection{3.2.2 Fee-Market Dampening}\label{fee-market-dampening}}

Following Easley, O'Hara \& Basu (2019) and Carter \& Jeng (2023), we
...
\subsubsection{3.2.3 Saturation Model}\label{saturation-model}}
...
\subsection{\texorpdfstring{3.3 Hashrate Concentration Analysis
(\texttt{hashrate\_concentration.py})}{3.3 Hashrate Concentration Analysis (hashrate\_concentration.py)}}\label{hashrate-concentration-analysis-hashrate_concentration.py}}

\hypertarget{metrics}{%
\subsubsection{3.3.1 Metrics}\label{metrics}}
...
\subsection{\texorpdfstring{3.4 Institutional Exit Cascade
(\texttt{institutional\_exit.py})}{3.4 Institutional Exit Cascade (institutional\_exit.py)}}\label{institutional-exit-cascade-institutional_exit.py}}

\hypertarget{model-description-hypothetical-extreme-stress-scenario}{%
\subsubsection{3.4.1 Model Description --- Hypothetical Extreme Stress
...
\subsubsection{3.4.2 Market Impact Model
...
empirical estimates from Makarov \& Schoar (2020) and the broader market
microstructure literature on illiquidity premia (Amihud, 2002) and price
impact modeling (Bouchaud et al., 2008). Note that \$30B daily volume
reflects normal market conditions; Makarov \& Schoar (2020) document
that liquidity can evaporate during systemic stress, with effective
market depth dropping by 50-80\%, substantially amplifying realized
impact.

\hypertarget{cascade-dynamics}{%
\subsubsection{3.4.3 Cascade Dynamics}\label{cascade-dynamics}}
...
\subsection{4.1 Miner Capitulation}\label{miner-capitulation}}

\textbf{Table 2: Public Miner Breakeven Prices and Financial Health
(Mid-2025).} Estimated breakeven BTC prices, EBITDA margins, and
leverage ratios for six major public miners. CleanSpark shows the lowest
breakeven at \$49,470, while Cipher Mining faces the highest at \$68,816
with negative margins, indicating acute vulnerability to price declines
below \$70K.
...
\caption{Figure 1: Miner capitulation stress test under price decline
scenarios with DAA feedback. The upper panel plots per-miner
capitulation probability (0--100\%) against BTC price (\$20K--\$100K)
for six publicly traded miners (WULF, CIFR, CORZ, MARA, RIOT, CLSK),
showing logistic curves that decline as price increases, with CIFR and
MARA exhibiting the highest vulnerability across most of the price
range. The lower panel displays the aggregate network security
...
\subsection{4.2 Content Pollution}\label{content-pollution}}
...
\caption{Figure 2: Content pollution probability and saturation curve
with fee-market dampening. The figure comprises four subplots:
(top-left) observed toxic block probability over time with a logistic
saturation fit converging to a 39.7\% asymptote, showing rapid growth
after late 2022 that plateaus by mid-2023; (top-right) inscription
density per block over time, with bars rising from near-zero pre-2023 to
peaks exceeding 100 inscriptions per block; (bottom-left) the fee-market
pushback multiplier as a function of average fee rate, illustrating how
the dampening factor approaches zero at high fee levels and thereby
suppresses low-value inscriptions; (bottom-right) the composite
pollution score decomposed by weighted factor contributions. The
saturation well below 100\% confirms that the fee-auction mechanism
...
\subsection{4.3 Hashrate Concentration}\label{hashrate-concentration}}

\textbf{Table 3: Hashrate Concentration and Censorship Resistance
Metrics.} Current network decentralization indicators showing that only
3 entities control majority hashrate (Nakamoto Coefficient = 3). The
51\% attack cost of \textasciitilde\$5.9B/yr, while substantial, remains
within reach of nation-state actors.
...
analysis (Vernetti, 2023; Cong, He \& Li, 2021).
...
\caption{Figure 3: Hashrate concentration and censorship resistance
metrics. The figure contains four panels: (top-left) a pie chart of
mining pool hashrate distribution showing Foundry USA at 30\%, AntPool
at 18\%, and F2Pool at 12\%, visually demonstrating that three entities
control a majority of network hashrate; (top-right) a horizontal bar
chart of geographic hashrate distribution color-coded by regulatory
risk, with the United States (\textasciitilde38\%) and China
...
\subsection{4.4 Institutional Exit Cascade (Hypothetical Extreme Stress
...
\caption{Figure 4: Institutional exit cascade simulation with
Almgren-Chriss market impact. The figure presents four panels:
...
\subsection{5.1 Parameter Sensitivity}\label{parameter-sensitivity}}
...
\subsection{5.2 Key Sensitivities}\label{key-sensitivities}}
...
\subsection{5.3 Robustness}\label{robustness}}
...
\subsection{6.1 Interconnected Risks}\label{interconnected-risks}}
...
\subsection{6.2 Limitations}\label{limitations-1}}
...
\item
  ...
  trading volume is assumed constant at \$30B, but Makarov \& Schoar
...
\subsection{6.3 Miner-to-AI/HPC Pivot}\label{miner-to-aihpc-pivot}}
...
popular analysis (e.g., ``Bitcoin Miners Are Abandoning BTC,'' YouTube,
2025), reflects the economic reality that AI/HPC hosting can offer more
stable and higher-margin revenue than Bitcoin mining, particularly
during periods of low BTC prices or post-halving margin compression.
...
\subsection{6.4 Implications for
...
\subsection{6.5 Implications for Bitcoin
...
\item
  ...
  transactions. \emph{Journal of Risk}, 3(2), 5--39.
\item
...
\item
  ...
  digest changes in supply and demand. In T. Hens \& K. Schenk-Hoppé
...
\item
  ...
  centralized pools. \emph{Review of Financial Studies}, 34(3),
  1191--1235.
\item
...
\item
  Gencer, A. E. et al.~(2018). Decentralization in Bitcoin and Ethereum
  networks. \emph{Proceedings of NDSS 2018}.
\item
...
\item
  ...
  \emph{bitcoin.org/bitcoin.pdf}.
\item
  Noda, A. (2020). On the evolution of cryptocurrency market efficiency.
  \emph{Applied Economics Letters}, 28(6), 433--439.
\item
  Prat, J. \& Walter, B. (2021). An equilibrium model of the market for
  Bitcoin mining. \emph{Journal of Political Economy}, 129(8),
  2415--2452.
\item
  Vernetti, F. (2023). Stratum V2: The next generation protocol for
  pooled mining. \emph{Braiins Technical Documentation}.
\item
...
\texttt{https://github.com/IngoGiebel/powsec-models}
...
\textbf{Table 5: Miner Stress Model Parameters.} Input parameters for
the miner capitulation and difficulty adjustment model, sourced from
blockchain data and industry reports.
...
\textbf{Table 6: Content Pollution Model Parameters.} Parameters
governing the inscription density, fee pushback, and flagging
...
\textbf{Table 7: Hashrate Concentration Model Parameters.} Data sources
for mining pool shares, geographic distribution, and regulatory
compliance estimates used in the decentralization assessment.
...
\textbf{Table 8: Institutional Exit Cascade Parameters.} Calibration
values for the market impact model governing ETF-driven selling
cascades, including temporary and permanent price impact coefficients.
